...
class Trainer:
    def __init__(self, model: nn.Module, lr: float = 5e-5, logger: Logger = None,
                 model_type: str = "transformer", use_focal_loss: bool = True) -> None:
        self.logger = logger
        self.model = model
        self.lr = lr
        ...
        self.criterion = FocalLoss().to(self.device) if self.use_focal_loss else nn.CrossEntropyLoss().to(self.device)
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.lr)
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode="min", factor=0.75, patience=6, threshold=0.001)
    ...
    def train(self, train_loader: torch.utils.data.DataLoader, val_loader: torch.utils.data.DataLoader,
              epochs: int = 20, output_path: str = "../output", save_model: bool = False) -> None:
        torch.manual_seed(0)
